...
def _tensor_matrix_multiply(
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    for ba in range(batch):
        a_batch = 0 if a_shape[0] == 1 else ba * a_strides[0]
        b_batch = 0 if b_shape[0] == 1 else ba * b_strides[0]
        out_batch = ba * out_strides[0]

        for i in range(m):
            for j in range(n):
                acc = 0.0
                for t in range(k):
                    a_pos = a_batch + i * a_strides[1] + t * a_strides[2]
                    b_pos = b_batch + t * b_strides[1] + j * b_strides[2]
                    acc += a_storage[a_pos] * b_storage[b_pos]

                out_pos = out_batch + i * out_strides[1] + j * out_strides[2]
                out[out_pos] = acc
