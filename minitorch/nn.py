...
def avgpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    return (x.sum(4) / (kh * kw)).view(batch, channel, new_height, new_width)
...
class Max(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        input, dim_val = ctx.saved_values
        return argmax(input, dim_val) * grad_output, 0.0
